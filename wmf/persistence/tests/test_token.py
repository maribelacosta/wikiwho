#!/usr/bin/python
import unittest
from .. import Token, Revision
import hashlib

def suite():
	suite = unittest.TestSuite(
		[
			unittest.TestLoader().loadTestsFromTestCase(TokenTestCase)
		]
	)
...
class TokenTestCase(unittest.TestCase):
	...
	def testIncrementAndVisible(self):
		rev = {
			"revid": 336293237,
			"pageid": 15661504,
			"parentid": 333114931,
		...
		w.increment(Revision(revId=336293238))
		w.increment(Revision(revId=336293239))
		...
		self.assertEqual(
			w.getRevisionIds(),
			[336293237, 336293238, 336293239]
		)
		
	def testGetAddedAndGetMostRecent(self):
		rev = {
			"revid": 336293237,
			"pageid": 15661504,
			"parentid": 333114931,
	...
	def testEq(self):
		...
		r1 = Revision.fromWPAPI(rev1)
		r2 = Revision.fromWPAPI(rev2)
		w1 = Token("Hello", r1)
		w2 = Token("Hello", r2)
		...
		w2.increment(Revision(revId=1234567890))
		self.assertNotEqual(
			w1,
			w2
		)
